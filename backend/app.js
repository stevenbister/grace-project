...
fs.readFile(text, 'utf-8', (err, data) => {
  ...
  const newLines = /(\r\n|\n|\r)/gm;
  const sortCode = /((sort code):\s(\d{2}-\d{2}-\d{2}))/gi;
  const accountNumber = /((Account Number):\s(\d{8}))/gi;
  ...
  const formatLines = lines =>
    lines
      ...
      // Lets also replace the bank deets for safety
      .replace(sortCode, '$2 xx-xx-xx')
      .replace(accountNumber, '$2 xxxxxxxx');

  // Add a tab to the end of each line
  const pushLinesIntoArray = lines =>
    formatLines(lines)
      .split('\r\n')
      .map(line => `${line}\t`);
  ...
  const splitLinesOnTab = lines =>
    lines.map(line => line.split('\t')).filter(line => line.splice(-1, 1));
  ...
  fs.writeFile('whatsapp-chat.json', JSON.stringify(output), 'utf-8', err => {
    if (err) throw err;
